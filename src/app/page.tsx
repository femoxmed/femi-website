...
export default function Home() {
  return (
    <main className="mesh-bg min-h-screen overflow-hidden font-sans text-white">
      ...
      <section id="top" className="relative mx-auto grid max-w-7xl gap-14 px-6 pb-20 pt-36 lg:grid-cols-[1.1fr_0.9fr] lg:px-8 lg:pt-44">
        ...
        <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7 }}>
          <div className="mb-8 inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-sm text-sky-100 shadow-2xl backdrop-blur-xl">
            ...
            Founder • Software Engineer • Product & Project Leader
          </div>

          <h1 className="font-display text-5xl font-black leading-[0.95] tracking-tight md:text-7xl lg:text-8xl">
            I build bold digital products with <span className="text-gradient">real impact.</span>
          </h1>

          <p className="mt-8 max-w-2xl text-lg leading-8 text-white/72 md:text-xl">
            I’m Femi Meduoye, founder of Art Change Lives and a technology leader focused on scalable websites, marketplaces, AI platforms, cloud solutions, dashboards, and business transformation.
          </p>

          <div className="mt-10 flex flex-wrap gap-4">
            <a href="#work" className="rounded-2xl bg-sky-300 px-7 py-4 font-bold text-slate-950 shadow-[0_0_45px_rgba(56,189,248,.35)] transition hover:-translate-y-1 hover:bg-white">
              View My Work
            </a>
            <a href="https://artchangelives.org" target="_blank" className="rounded-2xl border border-white/15 bg-white/10 px-7 py-4 font-bold backdrop-blur-xl transition hover:-translate-y-1 hover:bg-white/15">
              Art Change Lives
            </a>
          </div>

          <div className="mt-10 grid gap-3 text-sm text-white/70 sm:grid-cols-3">
            <a className="glass rounded-2xl p-4 transition hover:-translate-y-1" href="mailto:femi@artchangelives.org"><Mail className="mb-3 h-5 w-5 text-sky-300" />femi@artchangelives.org</a>
            <a className="glass rounded-2xl p-4 transition hover:-translate-y-1" href="tel:+2348099858115"><Phone className="mb-3 h-5 w-5 text-emerald-300" />+234 809 985 8115</a>
            <div className="glass rounded-2xl p-4"><MapPin className="mb-3 h-5 w-5 text-fuchsia-300" />Lagos, Nigeria</div>
          </div>
        </motion.div>
            ...
            <div className="rounded-[2rem] border border-white/10 bg-slate-950/70 p-6">
              <div className="mb-6 flex items-center justify-between">
                <div>
                  ...
                  <h2 className="font-display text-2xl font-bold">9 Selected Builds</h2>
                </div>
              ...
              <div className="space-y-4">
                {projects.slice(0, 5).map((project) => (
                  <a key={project.title} href={project.url} target="_blank" className="group flex items-center justify-between rounded-2xl border border-white/10 bg-white/[0.04] p-4 transition hover:border-sky-300/40 hover:bg-white/[0.08]">
                    <div>
      ...
      <div className="border-y border-white/10 bg-white/[0.04] py-4">
        <div className="animate-marquee whitespace-nowrap text-sm font-bold uppercase tracking-[0.35em] text-white/45">
          <span className="mx-8">AI Products</span><span className="mx-8">Marketplaces</span><span className="mx-8">Cloud Platforms</span><span className="mx-8">Dashboards</span><span className="mx-8">Startup Innovation</span><span className="mx-8">Digital Transformation</span>
          <span className="mx-8">AI Products</span><span className="mx-8">Marketplaces</span><span className="mx-8">Cloud Platforms</span><span className="mx-8">Dashboards</span><span className="mx-8">Startup Innovation</span><span className="mx-8">Digital Transformation</span>
        </div>
      </div>
      ...
      <section id="expertise" className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
        <div className="mb-12 max-w-3xl">
          <p className="mb-4 text-sm font-bold uppercase tracking-[0.35em] text-sky-300">Expertise</p>
          <h2 className="font-display text-4xl font-black md:text-6xl">Strategy, engineering, and execution in one profile.</h2>
        </div>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {strengths.map(({ title, icon: Icon }) => (
            <motion.div whileHover={{ y: -8 }} key={title} className="glass rounded-[2rem] p-7">
              ...
              <p className="mt-3 leading-7 text-white/60">Building reliable, polished, scalable solutions that support business goals and create a strong user experience.</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section id="work" className="mx-auto max-w-7xl px-6 py-24 lg:px-8">
        <div className="mb-12 flex flex-col justify-between gap-6 md:flex-row md:items-end">
          <div>
            <p className="mb-4 text-sm font-bold uppercase tracking-[0.35em] text-fuchsia-300">Selected Work</p>
            <h2 className="font-display text-4xl font-black md:text-6xl">Portfolio Projects</h2>
          </div>
          <p className="max-w-xl leading-7 text-white/65">A polished collection of platforms across art, commerce, AI, blockchain, cloud, productivity, automotive, and analytics.</p>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {projects.map((project, index) => (
            ...
            >
              <div className={`absolute -right-16 -top-16 h-40 w-40 rounded-full bg-gradient-to-br ${project.color} opacity-30 blur-2xl transition group-hover:opacity-60`} />
              <div className={`mb-8 h-3 w-24 rounded-full bg-gradient-to-r ${project.color}`} />
              <div className="relative flex items-start justify-between gap-4">
                <div>
                  <span className="rounded-full border border-white/10 bg-white/10 px-3 py-1 text-xs text-white/60">{project.tag}</span>
              ...
              <p className="relative mt-5 leading-7 text-white/62">{project.description}</p>
            </motion.a>
          ))}
        </div>
      </section>
      ...
      <section id="contact" className="mx-auto max-w-7xl px-6 pb-24 lg:px-8">
        <div className="glass overflow-hidden rounded-[2.5rem] p-8 text-center md:p-14">
          <p className="mb-5 text-sm font-bold uppercase tracking-[0.35em] text-amber-200">Let’s Connect</p>
          <h2 className="mx-auto max-w-4xl font-display text-4xl font-black md:text-6xl">Need a beautiful product, platform, or digital experience?</h2>
          <p className="mx-auto mt-6 max-w-2xl leading-8 text-white/65">Reach out for collaborations, product builds, founder partnerships, consulting, or technology leadership opportunities.</p>
          <div className="mt-10 flex flex-wrap justify-center gap-4">
            <a href="mailto:femi@artchangelives.org" className="rounded-2xl bg-white px-8 py-4 font-bold text-slate-950 transition hover:-translate-y-1 hover:bg-sky-200">Email Me</a>
            <a href="tel:+2348099858115" className="rounded-2xl border border-white/15 bg-white/10 px-8 py-4 font-bold transition hover:-translate-y-1 hover:bg-white/15">Call +234 809 985 8115</a>
          </div>
        </div>
      </section>
    </main>
